...
Segment.prototype.retrieveCrossDomainId = function(onCrossDomainIdReady) {
  ...
  if (!crossDomainId) {
    ...
    // TODO: check if this is copy-safe
    var domains = this.options.crossDomainIdServers;
    var currentTld = getTld(window.location.hostname);
    var currentTldindex = domains.indexOf(currentTld);
    // Should never happen due to server side config, but we'll protect against crashing here
    if (currentTldindex > -1) {
      domains.splice(currentTldindex, 1);
    }
    getCrossDomainIdFromServers(domains, writeKey, function(res) {
...
Segment.prototype.normalize = function(msg) {
  ...
  var crossDomainId = this.cookie('segment_cross_domain_id');
  if (!ctx.crossDomainId && crossDomainId) {
    ctx.crossDomainId = crossDomainId;
  }
 ...
 */
function getCrossDomainIdFromServers(domains, writeKey, callback) {
  var crossDomainIdFound = false;
  var finishedRequests = 0;
  for (var i=0; i<domains.length; i++) {
    var domain = domains[i];
    // For v0 we're just gonna grab the first id that came back
    getCrossDomainId(domain, writeKey, function(res) {
      finishedRequests++;
      if (res.id && !crossDomainIdFound) {
 ...
 * getCrossDomainId
 */
function getCrossDomainId(domain, writeKey, callback) {
