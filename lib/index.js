 ...
 * @api private
 */
Segment.prototype.retrieveCrossDomainId = function() {
  var crossDomainId = this.cookie('s:context.crossDomainId');
  if (!crossDomainId) {
    ...
    for (var i=0; i<domain; i++) {
      ...
      getJsonWithAuth(endpoint, writeKey, '', function(err, resJson) {
        ...
        if (resJson && resJson.id && !crossDomainIdFound) {
          ...
          this.cookie('s:context.crossDomainId', crossDomainId);
          this.analytics.identify({
            crossDomainId: crossDomainId
          });
          this.analytics.track('Cross Domain ID Obtained', {
            crossDomainId: crossDomainId,
            fromDomain: domain,
          ...
          this.analytics.track('Cross Domain ID Generated', {
            crossDomainId: crossDomainId,
            currentDomain: window.location.hostname,
          });
        }
