...
var Segment = exports = module.exports = integration('Segment.io')
  .option('apiKey', '') // Primary key
  .option('secondaryApiKeys', {}) // Secondary key mapping -> Might merge this and apiKey into one.
...
Segment.prototype.normalize = function(msg) {
  ...
  delete msg.options;
  // Use secondary write key if defined.
  var secondaryKey = this.options.secondaryApiKeys[msg._overwriteKey?] // Figure out API for this.
  msg.writeKey = secondaryKey || this.options.apiKey;
  ...
  // If we're not using the primary, everything is "unbundled"
  if (secondaryKey) {
    msg._metadata = msg._metadata || {};
    msg._metadata.bundled = [];
    msg._metadata.unbundled = bundled.concat(this.options.unbundledIntegrations);
  } else if (this.options.addBundledMetadata) {
...
Segment.prototype.retrieveCrossDomainId = function(callback) {
  ...
  if (!this.cookie('seg_xid')) {
    ...
    // TODO: Confirm this is main apiKey only.
